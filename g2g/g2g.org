...
cross-reference annotation. From that we pick the gene and protein IDs
by calling \ty{pick.awk}, which we still need to write.
#+end_export
#+begin_src sh <<g2g>>=
  awk '$3 == "CDS"' hs.gff |
      tr ';' '\n' |
...
look for the keys ``GeneID'' for the gene ID and ``GenBank'' for the
protein ID. Then we store these two values in a hash map.
#+end_export
