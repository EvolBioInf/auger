...
of \ty{N}, \ty{macle} returns $\cm=-1$. Figure~\ref{fig:mac} shows
example output from a sliding window analysis with 10 kb windows,
hence the first midpoint is 5000; in each round the window advances by
...
\end{minipage}
  ...
  \end{center}
\caption{Example output of \ty{macle}.}\label{fig:mac}
\end{figure}
...
the user to the program \ty{mantile}.
#+end_export
#+begin_src go <<Respond to options, Ch. \ref{ch:me}>>=
  if *optT == 0.0 {
...
analyze each window scanned. After the iteration over the windows,
there might still be one last interval to output.
#+end_export
...
in Figure~\ref{fig:mac}---name, midpoint, and $\cm$---and retrieve the
name of the current sequence. Then we retrieve the other two values
that make up the interval, the midpoint and the $\cm$. Based on the
current window, we have three decisions to take: whether or not to
...
window start and end. If $\cm < 0$, there is no data for this window
